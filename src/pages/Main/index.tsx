...
import { touchHistoryItem } from "@/utils/history";
...
const Main = () => {
  ...
  useKeyPress(shortcut.pastePlain, (event) => {
    ...
    const touched = touchHistoryItem(state.list, data.id);

    if (clipboardStore.content.autoSort && touched) {
      state.list = touched.list;
      updateHistory(data.id, { createTime: touched.createTime });
    }
